...
#
# Usage:
#   source ~/.claude-auto-speak/lib/session-detect.sh
...
SESSION_CONFIG_FILE="${INSTALL_DIR}/config.json"
SESSION_LOG_FILE="${INSTALL_DIR}/session-detect.log"
...
# Export functions
export -f count_user_messages 2>/dev/null || true
export -f is_skip_session_start_enabled 2>/dev/null || true
export -f get_min_user_messages 2>/dev/null || true
export -f should_skip_session_start 2>/dev/null || true
export -f session_log 2>/dev/null || true
